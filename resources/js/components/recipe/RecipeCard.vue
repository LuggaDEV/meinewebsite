...
<template>
    ...
    >
        <article class="group w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md transition-shadow duration-300 hover:border-slate-300 hover:shadow-xl">
            <Link :href="`/recipe/${recipe.id}`" class="block rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-terracotta)] focus-visible:ring-offset-2 focus-visible:ring-offset-white">
                ...
                <div class="border-t border-slate-100 p-5 md:p-6">
                    <h3 class="font-heading text-lg font-semibold text-[var(--color-forest)] transition-colors group-hover:text-[var(--color-terracotta)] md:text-xl">
                        {{ recipe.title }}
                    </h3>
                    <p class="mt-2 line-clamp-2 text-sm leading-relaxed text-[var(--color-warm-gray)]">
                        {{ recipe.description }}
                    </p>
